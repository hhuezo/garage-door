@php
    $linkClass =
        $linkClass ??
        'w-8 h-8 rounded-full bg-primary flex items-center justify-center text-white hover:scale-110 transition-transform';
    $socialLinks = $socialLinks ?? [];
@endphp
@if (! empty($socialLinks['instagram']))
    <a class="{{ $linkClass }}" href="{{ $socialLinks['instagram'] }}" target="_blank" rel="noopener noreferrer"
...
@if (! empty($socialLinks['facebook']))
    <a class="{{ $linkClass }}" href="{{ $socialLinks['facebook'] }}" target="_blank" rel="noopener noreferrer"
        aria-label="Facebook">
        <svg class="h-5 w-5 fill-current" viewBox="0 0 24 24" aria-hidden="true">
            <path
...
@if (! empty($socialLinks['tiktok']))
    <a class="{{ $linkClass }}" href="{{ $socialLinks['tiktok'] }}" target="_blank" rel="noopener noreferrer"
        aria-label="Tiktok">
        <svg class="h-5 w-5 fill-current" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#FFFF">
